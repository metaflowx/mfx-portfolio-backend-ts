...
export const signupService = async (userData: any) => {
    let { email, mobile, password, confirmPassword } = userData;
    ...
    /// Create the user in database
     const newUser = await UserRepository.create({ ...userData, password: hashedPassword,  otp: otp.toString(), otpExpire: new Date(Date.now() + 5 * 60 * 1000) });
